...
def query_images_by_location_and_properties(
    ...
    if cameraID is not None:
        optional_patterns.append(f'?image <https://exiftool.org/TagNames/EXIF.html#SerialNumber> "{cameraID}" .')
    ...
    if platform_id is not None:
        optional_patterns.append(f'?image <https://w3id.org/agri-image/platformID> "{platform_id}" .')
    ...
    if base_xyz_min and base_xyz_max:
        ...
        filters.append(f'FILTER(?baseX >= {x_min} && ?baseX <= {x_max} && ?baseY >= {y_min} && ?baseY <= {y_max} && ?baseZ >= {z_min} && ?baseZ <= {z_max})')
    ...
    if base_xyz_min and base_xyz_max:
        print(f"   Filter: Location X[{base_xyz_min[0]}, {base_xyz_max[0]}], "
